...
trait Templates {
  ...
  protected lazy val templates: Map[String, Body with Expression[String]] =
    // EVERY `templates` root, not just the first: sbt 2 puts resource directories on the classpath
    // as-is, behind a class directory that can hold an empty `templates/` of its own.
    Thread.currentThread.getContextClassLoader.getResources("templates").asScala.toList match {
      case Nil       =>
      ...
      case resources =>
        // An earlier classpath root wins a name clash, as it would for any other resource.
        resources.reverse.flatMap(templatesIn).toMap
    }

  private def templatesIn(resource: java.net.URL): List[(String, Body with Expression[String])] = {
