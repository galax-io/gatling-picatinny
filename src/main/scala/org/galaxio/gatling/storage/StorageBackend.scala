...
/** JDBC-backed session storage.
  *
  * The auto-created table uses a dialect-aware identity column so `initTable` works across engines: MySQL/MariaDB get
  * `AUTO_INCREMENT`, while PostgreSQL, H2 and other SQL-standard engines get `GENERATED BY DEFAULT AS IDENTITY`. The identity
  * `id` is what `load()` orders by to guarantee insertion order, so it is kept on every dialect. Integration coverage runs
  * against PostgreSQL (see `JdbcStorageIntegrationSpec`).
  */
