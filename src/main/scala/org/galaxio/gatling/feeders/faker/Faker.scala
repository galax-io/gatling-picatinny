...
object Faker {
  ...
  object finance {
    ...
    /** Generates a valid IBAN with correct ISO 7064 Mod 97-10 check digits via iban4j for IBAN-system countries. For countries
      ...
      */
    def iban(country: Country = Country.DE): Generator[String] =
      for {
        fallbackBody <- string.numeric(20)
      } yield {
        val generated =
          for {
            code <- scala.util.Try(org.iban4j.CountryCode.valueOf(country.iso2))
            iban <- scala.util.Try(org.iban4j.Iban.random(code).toString)
          } yield iban
        generated.getOrElse(s"${country.iso2}00$fallbackBody")
      }

    /** EAN-13 barcode (12 digits + ISO check digit, computed via Apache Commons Validator). */
    def ean13(): Generator[String] =
      for {
        body <- string.numeric(12)
      } yield body + EAN13CheckDigit.EAN13_CHECK_DIGIT.calculate(body)

    /** ISBN-13 book number (`978`/`979` prefix + 9 digits + check digit). */
    def isbn13(): Generator[String] =
      for {
        prefix <- oneOf("978", "979")
        body   <- string.numeric(9)
      } yield {
        val first12 = s"$prefix$body"
        first12 + ISBNCheckDigit.ISBN13_CHECK_DIGIT.calculate(first12)
      }

    /** ISBN-10 book number (9 digits + mod-11 check digit, which may be `X`). */
    def isbn10(): Generator[String] =
      for {
        body <- string.numeric(9)
      } yield body + ISBNCheckDigit.ISBN10_CHECK_DIGIT.calculate(body)
  ...
  object ru {
    ...
    /** Russian vehicle registration plate (гос. номер), e.g. `А123ВС77` — letter, 3 digits, 2 letters, region. */
    def licensePlate(): Generator[String] =
      for {
        l1     <- oneOf(plateLetters)
        digits <- number.int(0, 999)
        l2     <- oneOf(plateLetters)
        l3     <- oneOf(plateLetters)
        region <- number.int(1, 99)
      } yield f"$l1$digits%03d$l2$l3$region%02d"
  }

  /** Polish identifiers. */
  object pl {

    /** Polish PESEL generated by the `io.github.viepovsky:pesel` library (valid date encoding + check digit). */
    def pesel(): Generator[String] = Generator.delay(PeselGenerator.generatePeselStatic())
  }

  /** Swedish identifiers. */
  object se {

    /** Swedish personnummer `YYMMDDNNNC` (10 digits). The check digit `C` is a Luhn digit over the first 9 (computed via Apache
      * Commons Validator); the result passes `dev.personnummer` validation.
      */
    def personnummer(): Generator[String] =
      for {
        yy     <- number.int(0, 99)
        mm     <- number.int(1, 12)
        dd     <- number.int(1, 28)
  ...
  /** Vehicle identifiers. */
  object vehicle {
    private val vinAlphabet = "ABCDEFGHJKLMNPRSTUVWXYZ0123456789" // excludes I, O, Q per ISO 3779

    /** Vehicle Identification Number (17 chars). The body is random over the legal VIN alphabet; the position-9 check digit is
      * computed by the `vin-utils` library so the value passes VIN validation.
      */
    def vin(): Generator[String] =
      for {
        body <- string.fromAlphabet(vinAlphabet, 17)
      } yield body.updated(8, VinValidatorUtils.getVinCheckSumChar(body))
  }

  /** Device identifiers. */
  object device {

    /** IMEI (15 digits): 14 random digits + a Luhn check digit computed via Apache Commons Validator. */
    def imei(): Generator[String] =
      for {
        body <- string.numeric(14)
      } yield body + LuhnCheckDigit.LUHN_CHECK_DIGIT.calculate(body)
  }

  /** Brazilian identifiers. */
  object br {
    private lazy val cpfValidator  = new StellaCpf()
    private lazy val cnpjValidator = new StellaCnpj()

    /** Brazilian CPF (individual) generated by the caelum-stella library — valid mod-11 check digits. */
    private val CpfFormatPattern = java.util.regex.Pattern.compile("(\\d{3})(\\d{3})(\\d{3})(\\d{2})")

    def cpf(formatted: Boolean = false): Generator[String] =
      Generator.delay {
        val raw = cpfValidator.generateRandomValid()
        if (formatted) CpfFormatPattern.matcher(raw).replaceFirst("$1.$2.$3-$4") else raw
      }

    /** Brazilian CNPJ (company) generated by the caelum-stella library — valid check digits. */
    def cnpj(formatted: Boolean = false): Generator[String] =
      Generator.delay {
        val raw = cnpjValidator.generateRandomValid()
        if (formatted) raw.replaceFirst("(\\w{2})(\\w{3})(\\w{3})(\\w{4})(\\d{2})", "$1.$2.$3/$4-$5") else raw
      }
  }
  ...
  object de {
    ...
    def steueridentifikationsnummer(): Generator[String] =
      Generator.delay {
        val rnd = ThreadLocalRandom.current()
        val sb  = new java.lang.StringBuilder(11)
        sb.append(('0' + rnd.nextInt(1, 10)).toChar)
        var i   = 0
        while (i < 10) {
          sb.append(('0' + rnd.nextInt(0, 10)).toChar)
          i += 1
        }
        sb.toString
      }
