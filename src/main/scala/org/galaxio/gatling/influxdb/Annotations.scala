...
object Annotations {
  ...
  def influxDataPoint(tagKey: String, tagValue: String, fieldKey: String, fieldValue: String): Unit = {
    AnnotationManager.addCustomAnnotation(tagKey, tagValue, fieldKey, fieldValue)
  }

  def influxDataPoint(point: Point): Unit = {
    AnnotationManager.addCustomPoint(point)
  }

  def influxDataPoint(point: Seq[Point]): Unit = {
