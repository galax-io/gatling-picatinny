...
import io.cosmospf.gatling.redis.RedisActionBuilder.*;
...
import static io.cosmospf.gatling.javaapi.internal.Expression.toListExpression;
...
public final class RedisClientPoolJava {
    ...
    public RedisActionBuilder<RedisDelActionBuilder> DEL(String key, String... keys) {
        return new RedisActionBuilder<>(
                new io.cosmospf.gatling.redis.RedisActionBuilder.RedisClientPoolOps(redisClientPoolAsScala()).DEL(
    ...
    public RedisActionBuilder<RedisSremActionBuilder> SREM(String key, String value, String... values) {
        return new RedisActionBuilder<>(
                new io.cosmospf.gatling.redis.RedisActionBuilder.RedisClientPoolOps(redisClientPoolAsScala()).SREM(
    ...
    public RedisActionBuilder<RedisSaddActionBuilder> SADD(String key, String value, String... values) {
        return new RedisActionBuilder<>(
                new io.cosmospf.gatling.redis.RedisActionBuilder.RedisClientPoolOps(redisClientPoolAsScala()).SADD(
                        toAnyExpression(key), toAnyExpression(value), toListExpression(values)
                )
        );
    }
