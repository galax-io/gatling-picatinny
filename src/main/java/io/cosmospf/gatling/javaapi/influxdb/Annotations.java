package io.cosmospf.gatling.javaapi.influxdb;
...
public final class Annotations {
    ...
    public static PopulationBuilder userDataPoint(String uniqScpName, Point point) {
        return new PopulationBuilder(io.cosmospf.gatling.influxdb.Annotations.userDataPoint(uniqScpName, point));
    }
    ...
    public static PopulationBuilder userDataPoint(String uniqScpName,
        ...
        return new PopulationBuilder(
                io.cosmospf.gatling.influxdb.Annotations.userDataPoint(
                        uniqScpName,
                        tagKey,
                        tagValue,
                        fieldKey,
    ...
    public static Function<Session, Session> userDataPoint(Point point) {
        return (Session session) -> {
            io.cosmospf.gatling.influxdb.Annotations.influxDataPoint(point);
            return session;
        };
    }
    ...
    public static Function<Session, Session> userDataPoint(String tagKey,
        ...
        return (Session session) -> {
            toStringExpression(tagValue).apply(session.asScala()).onSuccess((tag) -> {
                ...
                {
                    io.cosmospf.gatling.influxdb.Annotations.influxDataPoint(tagKey, tag, fieldKey, field);
                    return 0;
                });
                return 0;
            });
