  ...
  "JwtKeys" when {
    ...
    "loading keys from filesystem" should {
      ...
      "load RSA private key from file path" in {
        val path = getClass.getClassLoader.getResource("keys/rsa_private_pkcs8.pem").getPath
      ...
      "load RSA public key from file path" in {
        val path = getClass.getClassLoader.getResource("keys/rsa_public.pem").getPath
      ...
      "load EC private key from file path" in {
        val path = getClass.getClassLoader.getResource("keys/ec_private_pkcs8.pem").getPath
      ...
      "load EC public key from file path" in {
        val path = getClass.getClassLoader.getResource("keys/ec_public.pem").getPath
