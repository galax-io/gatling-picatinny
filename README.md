...
Clone the repo, open the `Debug` simulation for your language, and run it to confirm a green run before adding your own scenarios. For Scala, generate a project with the galaxio CLI first — see [Examples](#examples).
...
**Scala** — [examples/scala-sbt-example](examples/scala-sbt-example) is an overlay applied on top of a project generated by the galaxio CLI. There is no standalone `build.sbt` by design. Generate a fresh project with the CLI first, then run `sbt Gatling/test`.
