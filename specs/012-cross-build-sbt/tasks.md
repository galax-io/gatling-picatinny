...
- [X] T059 (**done 2026-08-21**, run [32450643390](https://github.com/galax-io/gatling-picatinny/actions/runs/32450643390) on a throwaway branch, since deleted: removed the `products` pin — an sbt-2-only break — and `workflow_dispatch`ed the job. sbt 2 = **723 passed / 2 failed** on `TemplatesSpec`, job named `sbt 2 compatibility (build change)`, failing step `Full gate suite`; sbt 1 on the same commit = **725/0, 824/824**. The gate is proven able to fail, and to fail on exactly one major) Demonstrate the anti-vacuity requirement (contract M-4, SC-007): introduce a change valid under sbt 1 and invalid under sbt 2, trigger `sbt2-compat.yml` via `workflow_dispatch`, confirm it fails and names the major, then revert. A job never seen to fail has not been shown to work
...
- [X] T074 (**no issue needed — already fixed upstream**: v0.15.0 defaulted `SbtGatlingVersion` 4.18.3 / `SbtVersion` 1.12.13; **v0.16.0** ships 4.19.1 / 1.12.15. Bumped `TEMPLATES_GATLING_VERSION` and both template scripts to v0.16.0 instead. The `--set` overrides stay as belt-and-braces against future default drift) ~~File an upstream issue against `galax-io/templates-gatling`~~ to bump the `scala-sbt` template's `SbtGatlingVersion` default from 4.18.3 to 4.19.1. **The blocker is in a third-party repository** — but with the overlay check now on a schedule rather than gating PRs, T072's `--set SbtGatlingVersion=4.19.1` fully works around it and this upstream fix is no longer on the critical path
