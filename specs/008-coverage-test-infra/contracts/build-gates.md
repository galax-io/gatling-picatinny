...
| Binary compatibility (advisory) | `sbt mimaFindBinaryIssues` | restore API, or acknowledge intentional break | NEVER blocks: local `Find` prints findings and exits green; CI runs `mimaReportBinaryIssues` under `continue-on-error: true` and re-emits findings as `::warning::` annotations vs baseline — workflow stays green; warnings reviewed at release | `mimaBinaryIssueFilters` entry + justification + version bump (constitution II) — keeps warning stream clean |
