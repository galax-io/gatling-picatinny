...
- [ ] T022 (all gates green EXCEPT coverage: 74.59% stmt < 75 floor — pre-existing drift on main, zero uncovered statements from this diff; separate decision required) Full gate run (contracts/parity-and-gates.md): `sbt scalafmtAll scalafmtSbt` then `sbt scalafmtCheckAll scalafmtSbtCheck "scalafixAll --check" compile test` + coverage ≥75/66 + `sbt mimaFindBinaryIssues` zero new findings (SC-001/SC-005)
