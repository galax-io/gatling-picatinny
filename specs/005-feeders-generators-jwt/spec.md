...
- **FR-001**: The `long(min, max)` generator MUST respect the full `Long` range — bounded calls whose limits exceed the `Int` range MUST stay within `[min, max]` and MUST NOT fall through to an unbounded draw. (`feeders/generators/GeneratorInstances.scala:24-31`: today gated by `Int.MaxValue`/`Int.MinValue`.)
- **FR-002**: The `feeders/generators` package MUST gain a unit test suite (`Test` scope, no container) covering numeric/string/character/collection generators, the `~`/`**`/`separateBy` combinators, seeded determinism, and case-class derivation, with exact-value and boundary assertions. (Package currently has zero tests.)
...
- **FR-005**: `claimFromSession` MUST infer the JSON claim type from the genuine session value: a numeric value → JSON number, a boolean value → JSON boolean, any other value (including a numeric-looking String) → JSON string. The library MUST additionally provide typed override entry points (e.g. force long/boolean/string) so an author can pin a specific JSON type regardless of the inferred one. (`utils/jwt/ClaimsBuilder.scala:107-118`: today every EL claim is wrapped in `JString`.)
...
- **FR-009**: Every behavioral fix in FR-001, FR-003–FR-007 MUST ship with at least one negative-case test verifying the rejection or corrected output, and at least one positive case confirming valid input is unaffected (constitution §III).
