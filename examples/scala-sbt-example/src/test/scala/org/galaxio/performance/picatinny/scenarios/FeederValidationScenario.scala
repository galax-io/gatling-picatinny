...
/** Business flow for the feeder-validation e2e (test-model layer 4). Flow only (galaxio-gatling-pro boundaries).
  *
  * A SINGLE `.feed` of the zipped picatinny faker feeder increments every value at once; one request then carries them all and
  * validates each echoed value against its expected pattern.
  */
